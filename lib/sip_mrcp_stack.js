...
class SipMrcpStack {
        ...
        this.sip_stack = sip.create(
            ...
            req => {
                try {
                    ...
                    log.debug(__line, uuid, `got SIP request ${req.method}`)
                    ...
                    if(req.method == 'CANCEL') {
                        ...
                        log.debug(__line, uuid, `Sending ${rs} ${rr} reply`)
                        ...
                        log.debug(__line, uuid, `refused with ${rs} ${rr} due to CANCEL`)
                        this.sip_stack.send(res)
                    ...
                    const res = sip.makeResponse(req, rs, rr)
                    this.sip_stack.send(res)
                    log.debug(__line, uuid, `$[req.method} refused with ${rs} ${rr}`)

                } catch(error) {
                    log.error(__line, 'sip_server', error)
                }
    ...
    _create_mrcp_server(mrcp_options) {
        this.mrcp_server = mrcp.createServer(conn => {
            conn.once('data', data => {
                ...
                log.debug(__line, 'mrcp_server', `got MRCP message ${JSON.stringify(data)}`)
                const mrcp_uuid = data.headers['channel-identifier']

                if(!this.mrcp2sip.hasOwnProperty(mrcp_uuid)) {
                    log.warn(__line, mrcp_uuid, `got unexpected MRCP message ${data} for unknown mrcp_uuid`)
                ...
                if(!session) {
                    ...
                    log.error(__line, mrcp_uuid, `unexpected MRCP request for non existing session ${uuid} ${JSON.stringify(data)}`)
                    const response = mrcp.builder.build_response(data.request_id, 405, 'COMPLETE', {'channel-identifier': mrcp_uuid})
                    utils.safe_write(conn, response)
                    conn.end()
                    return
            ...
            conn.on('error', err => {
                log.error(__line, 'mrcp_server', `conn error: ${err}. App will terminate.`)
        ...
        this.mrcp_server.on('error', err => {
                log.error(__line, 'mrcp_server', err)
                process.exit(1)
        })
    }
    ...
    add_new_session(uuid, data) {
        ...
        if(this.sessions[uuid]) {
            log.error(__line, uuid, `removing previous session with the same uuid`)
        ...
        session.on('error', err => {
            log.error(__line, uuid, `session error:${uuid}`)
            this.free_session(session)
        })

        session.on('refused', err => {
            log.error(__line, uuid, `session refused by app layer`)
            this.free_session(session)
        })
    ...
    free_session(session) {
        ...
        log.debug(__line, session.uuid, 'freeing session')
        ...
        if(session.rtp_session && session.data.local_rtp_port) {
            ...
            try {
                log.debug(__line, session.uuid, `closing rtp_session`)    
                session.rtp_session.close()
            } catch(err) {
                log.debug(__line, session.uuid, `failed to close rtp_session: ${err}`)    
            }

            log.debug(__line, session.uuid, `deallocated rtp port ${rtp_port}`)
        ...
        if(session.data.sip_session) {
            log.debug(__line, session.uuid, `sending BYE`)    
            this.sip_stack.send({
                method: 'BYE',
                ...
                log.debug(__line, session.uuid, `BYE got: ${res.status} ${res.reason}`)    
            })
        }

        if(session.mrcp_socket) {
            session.mrcp_socket.end()
            log.debug(__line, session.uuid, `closed mrcp socket`)
    ...
    process_incoming_call(uuid, req, new_session_callback) {
        ...
        if(!mrcp_utils.offer_sdp_matcher(offer_sdp, sdp_data)) {
            ...
            log.info(__line, uuid, `refused with ${rs} ${rr}`)
            return
        }
        ...
        log.debug(__line, uuid, `allocated rtp port ${local_rtp_port}`)
        ...
        const rr = 'Trying'
        this.sip_stack.send(sip.makeResponse(req, rs, rr))
        log.debug(__line, uuid, `${req.method} accepted with ${rs} ${rr}`)
    ...
    process_in_dialog_request(uuid, req) {
        ...
        if(req.method != 'INVITE' && req.method != 'BYE') {
            ...
            const rr = 'OK'
            const res = sip.makeResponse(req, rs, rr)
            log.debug(__line, uuid, `unexpected in-dialog ${req.method}. Sending default ${rs} ${rr} reply`)
            this.sip_stack.send(res)
            return
        }

        if(req.method == 'BYE') {
            log.debug(__line, uuid, 'received BYE')
            ...
            log.debug(__line, uuid, `replied BYE with ${rs} ${rr}`)

            var session = this.sessions[uuid]

            if(!session) {
                log.error(__line, uuid, `received BYE but session was not found`)
                return
            }
        ...
        log.error(__line, uuid, "REINVITE SUPPORT IMPLEMENTATION PENDING. APP WILL EXIT")
        process.exit(1)
    }
    ...
    constructor(sip_options, rtp_options, mrcp_options, new_session_callback) {
        ...
        try {
            ...
            log.error(__line, 'sip_mrcp.constructor', `error: ${error}`)
            process.exit(1) 
        }
    }
        ...
        this.sip_stack.send(
            ...
            rs => {
                if(rs.status >= 300) {
                    ...
                    log.info(__line, uuid, `call progress status ${rs.status} ${rs.reason}`)
                } else {
                    // yes we can get multiple 2xx response with different tags
                    log.info(__line, uuid, `call answered with tag ${rs.headers.to.params.tag}`)
    ...
    accept(session) {
        ...
        this.sip_stack.send(res,
            function(res) {
                log.info(__line, session.uuid, 'got callback to res sent to out-of-dialog INVITE on sip stack')
            }
        )

        log.debug(__line, session.uuid, `INVITE accepted with ${rs} ${rr} by app layer`)
    }

    refuse(session, response_status, response_reason) {
        this.sip_stack.send(sip.makeResponse(session.data.sip_session, rs, rr))
        log.debug(__line, session.uuid, `INVITE refused with ${rs} ${rr} by app layer`)
