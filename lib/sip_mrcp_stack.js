...
class SipMrcpStack {
    ...
    free_session(session) {
        ...
        log(__line, 'debug', session.uuid, 'freeing session')
        ...
        if(session.rtp_session && session.data.local_rtp_port) {
            ...
            try {
                log(__line, 'debug', session.uuid, `closing rtp_session`)    
                session.rtp_session.close()
            } catch(err) {
                log(__line, 'debug', session.uuid, `failed to close rtp_session: ${err}`)    
            }
        ...
        if(session.data.sip_session) {
            log(__line, 'debug', session.uuid, `sending BYE`)    
            this.sip_stack.send({
                method: 'BYE',
                ...
                log(__line, 'debug', session.uuid, `BYE got: ${res.status} ${res.reason}`)    
            })
        }
