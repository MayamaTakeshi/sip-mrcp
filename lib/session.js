...
class Session extends EventEmitter {
    ...
    send_mrcp_msg(msg) {
        if(!this.mrcp_socket) {
            log(__line, 'error', this.uuid, 'mrcp_socket not ready') 
        }
    ...
    send_rtp_data(data) {
         if(!this.rtp_session) {
            log(__line, 'error', this.uuid, 'rtp_session not ready') 
        }

        this.rtp_session.send_payload(data)
    }
}
